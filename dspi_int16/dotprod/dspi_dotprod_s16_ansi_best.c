...
esp_err_t dspi_dotprod_s16_ansi(image2d_t *in_image, image2d_t *filter, int16_t *out_value, int count_x, int count_y, int shift)
{
    ...
    for (int y = 0; y < count_y; y++) {
        ...
        for (; x < count_x - 7; x += 8) {
            ...
            acc += (int32_t)i_data[in_image->step_x * (x+1)] * (int32_t)f_data[filter->step_x * (x+1)];
            acc += (int32_t)i_data[in_image->step_x * (x+2)] * (int32_t)f_data[filter->step_x * (x+2)];
            acc += (int32_t)i_data[in_image->step_x * (x+3)] * (int32_t)f_data[filter->step_x * (x+3)];
            acc += (int32_t)i_data[in_image->step_x * (x+4)] * (int32_t)f_data[filter->step_x * (x+4)];
            acc += (int32_t)i_data[in_image->step_x * (x+5)] * (int32_t)f_data[filter->step_x * (x+5)];
            acc += (int32_t)i_data[in_image->step_x * (x+6)] * (int32_t)f_data[filter->step_x * (x+6)];
            acc += (int32_t)i_data[in_image->step_x * (x+7)] * (int32_t)f_data[filter->step_x * (x+7)];
        }
        // 处理剩余的元素
        for (; x < count_x; x++) {
            acc += (int32_t)i_data[in_image->step_x * x] * (int32_t)f_data[filter->step_x * x];
