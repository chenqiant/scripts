    ...
    for (int i = 0; i < A_rows; i++) {
        for (int j = 0; j < B_cols; j++) {
            ...
            for (; s < A_cols - 7; s += 8) {
                ...
                C[i * C_step + j] += A[i * A_step + s + 1] * B[(s + 1) * B_step + j];
                C[i * C_step + j] += A[i * A_step + s + 2] * B[(s + 2) * B_step + j];
                C[i * C_step + j] += A[i * A_step + s + 3] * B[(s + 3) * B_step + j];
                C[i * C_step + j] += A[i * A_step + s + 4] * B[(s + 4) * B_step + j];
                C[i * C_step + j] += A[i * A_step + s + 5] * B[(s + 5) * B_step + j];
                C[i * C_step + j] += A[i * A_step + s + 6] * B[(s + 6) * B_step + j];
                C[i * C_step + j] += A[i * A_step + s + 7] * B[(s + 7) * B_step + j];
            }
            for (; s < A_cols; s++) {
                C[i * C_step + j] += A[i * A_step + s] * B[s * B_step + j];
            }
