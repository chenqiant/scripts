...
# -mllvm -filter-print-funcs=dsps_sqrt_f32_ansi

opt  dsps_sqrt_f32_ansi.ll -mtriple=riscv32-esp-unknown-elf -passes=riscv-split-loop-by-length -riscv-split-loop-by-length=true -S -o after_splitloopbylength.ll
...
llc --mcpu=esp32p4 --enable-esp32-p4-optimize --mtriple=riscv32 after_loopunrollandremainder.ll -O3 -filetype=asm -o after_loopunrollandremainder.s

../preprocess_asm.sh after_loopunrollandremainder.s

cp after_loopunrollandremainder.s /home/chenqian/esp/esp-idf/components/esp-dsp/modules/math/sqrt/float/dsps_sqrt_f32_arp4.S
