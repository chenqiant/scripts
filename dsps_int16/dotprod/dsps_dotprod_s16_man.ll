...
for.cond.cleanup.loopexit:                        ; preds = %for.body.clone, %for.body.7
  %add.lcssa = phi i64 [ %add, %for.body.7 ], [ %add.1, %for.body.7 ], [ %add.2, %for.body.7 ], [ %add.3, %for.body.7 ], [ %add.4, %for.body.7 ], [ %add.5, %for.body.7 ], [ %add.6, %for.body.7 ], [ %add.7, %for.body.7 ]
  %2 = add i32 %1, 8
...
for.cond73.preheader:                             ; preds = %entry, %for.cond.cleanup.loopexit
  ...
  %result0.0.lcssa = phi i64 [ %conv1, %entry ], [ %add.7, %for.body.7 ]
  %result0.0.lcssa1 = phi i64 [ 0, %entry ], [ %add.6, %for.body.7 ]
  %result0.0.lcssa2 = phi i64 [ 0, %entry ], [ %add.5, %for.body.7 ]
  %result0.0.lcssa3 = phi i64 [ 0, %entry ], [ %add.4, %for.body.7 ]
  %result0.0.lcssa4 = phi i64 [ 0, %entry ], [ %add.3, %for.body.7 ]
  %result0.0.lcssa5 = phi i64 [ 0, %entry ], [ %add.2, %for.body.7 ]
  %result0.0.lcssa6 = phi i64 [ 0, %entry ], [ %add.1, %for.body.7 ]
  %result0.0.lcssa7 = phi i64 [ 0, %entry ], [ %add, %for.body.7 ]
  ;%result0.0.lcssa8 = phi i32 [ 0, %entry ], [ %inc.7, %for.body.7 ]
  %cmp74172 = icmp slt i32 %i.0.lcssa, %len
  br i1 %cmp74172, label %for.body.clone, label %for.end85

for.body.7:                                       ; preds = %for.body.preheader, %for.body.7, %for.body.clone
  %acc.026 = phi i64 [ %conv1, %entry ], [ %add.7, %for.body.7 ]
  %result6 = phi i64 [ %conv1, %entry ], [ %add.6, %for.body.7 ]
  %result5 = phi i64 [ %conv1, %entry ], [ %add.5, %for.body.7 ]
  %result4 = phi i64 [ %conv1, %entry ], [ %add.4, %for.body.7 ]
  %result3 = phi i64 [ %conv1, %entry ], [ %add.3, %for.body.7 ]
  %result2 = phi i64 [ %conv1, %entry ], [ %add.2, %for.body.7 ]
  %result1 = phi i64 [ %conv1, %entry ], [ %add.1, %for.body.7 ]
  %result0 = phi i64 [ %conv1, %entry ], [ %add, %for.body.7 ]
  %i.025 = phi i32 [ 0, %entry ], [ %inc.7, %for.body.7 ]
  ...
  br i1 %exitcond.not.7, label %for.cond.cleanup.loopexit, label %for.body.7, !llvm.loop !8

for.body.clone:                                   ; preds = %for.cond73.preheader
  %acc.026.clone = phi i64 [ %conv1, %for.body.preheader ], [ %add.7, %for.body.7 ]
  %i.025.clone = phi i32 [ 0, %for.body.preheader ], [ %inc.7, %for.body.7 ]
  %arrayidx.clone = getelementptr inbounds i16, ptr %src1, i32 %i.025.clone
  %19 = load i16, ptr %arrayidx.clone, align 2, !tbaa !4
  %conv3.clone = sext i16 %19 to i32
  %arrayidx4.clone = getelementptr inbounds i16, ptr %src2, i32 %i.025.clone
  %20 = load i16, ptr %arrayidx4.clone, align 2, !tbaa !4
  %conv5.clone = sext i16 %20 to i32
  %mul.clone = mul nsw i32 %conv5.clone, %conv3.clone
  %conv6.clone = sext i32 %mul.clone to i64
  %add.clone = add nsw i64 %acc.026.clone, %conv6.clone
  %inc.clone = add nuw nsw i32 %i.025.clone, 1
  %exitcond.not.clone = icmp eq i32 %inc.clone, %len
  br i1 %exitcond.not.clone, label %for.cond.cleanup.loopexit, label %for.body.7, !llvm.loop !8

for.end85:                                        ; preds = %for.cond73.preheader
