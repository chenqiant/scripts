...
# 提取函数
opt -S -filter-print-funcs=dsps_mul_f32_ansi dsps_mul_f32_ansi.ll -o extracted_function.ll
...
opt -filter-print-funcs=dsps_mul_f32_ansi -mtriple=riscv32-esp-unknown-elf -passes=riscv-split-loop-by-length -riscv-split-loop-by-length=true after_clean_metadata_funcattr.ll  -S -o after_splitloopbylength.ll
...
opt -filter-print-funcs=dsps_mul_f32_ansi -mtriple=riscv32-esp-unknown-elf -passes=riscv-custom-licm -riscv-custom-licm=true after_splitloopbylength.ll  -S -o after_customlicm.ll
